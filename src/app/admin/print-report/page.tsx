  ...
  <p style={{ marginTop: "6px" }}>Senior dom Svida</p>
      ...
      <table style={tableStyle}>
        <tbody>
          ...
          <tr>
            <td style={cellTitle}>Počet odoslaných hodnotení</td>
            <td style={cellValue}>{usedEvaluations || 0}</td>
          </tr>

          <tr>
            <td style={cellTitle}>Počet hodnotených zamestnancov</td>
            <td style={cellValue}>{evaluatedEmployees.length}</td>
          </tr>

          <tr>
            <td style={cellTitle}>Počet sledovaných zamestnancov</td>
            <td style={cellValue}>{employees?.length || 0}</td>
          </tr>

          <tr>
            <td style={cellTitle}>Celkový priemer hodnotenia</td>
            <td style={cellValue}>
          ...
          <tr>
            ...
            <td style={cellValue}>{getScoreLevel(overallAverage)}</td>
          </tr>
        </tbody>
      </table>
        ...
        <table style={tableStyle}>
          <thead>
            <tr>
              ...
              <th style={cellTitle}>Počet hodnotení</th>
            </tr>
          </thead>
          ...
          <tbody>
            {topEmployees.map((employee: any, index: number) => (
              <tr key={employee.id}>
                ...
                <td style={cellValue}>{getPositionName(employee)}</td>
                <td style={cellValue}>{employee.average.toFixed(2)}</td>
                <td style={cellValue}>{employee.evaluationCount}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ) : (
        <p>Zatiaľ nie sú dostupné hodnotenia zamestnancov.</p>
      )}

      <h3 style={subSectionTitle}>Najnižšie hodnotenie</h3>
      ...
      <h2 style={sectionTitle}>3. Najsilnejšie oblasti</h2>
          ...
          <tbody>
            {strongestCategories.map((category) => (
              <tr key={category.categoryName}>
                <td style={cellValue}>{category.categoryName}</td>
                <td style={cellValue}>{category.average.toFixed(2)}</td>
                <td style={cellValue}>{category.count}</td>
                <td style={cellValue}>{getScoreLevel(category.average)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ) : (
        <p>Zatiaľ nie sú dostupné kategórie hodnotenia.</p>
      )}

      <h2 style={sectionTitle}>4. Najrizikovejšie oblasti</h2>
      ...
      <h2 style={sectionTitle}>5. Podklad pre ročný plán vzdelávania</h2>

      {trainingPlanSummary.length > 0 ? (
        <table style={tableStyle}>
          <thead>
            <tr>
              <th style={cellTitle}>Vzdelávacia téma</th>
              <th style={cellTitle}>Počet zamestnancov</th>
              <th style={cellTitle}>Dôvod zaradenia</th>
              <th style={cellTitle}>Odporúčaná forma</th>
              <th style={cellTitle}>
                Vyhodnotenie účelnosti a prenosu do praxe
              </th>
            </tr>
          </thead>
          ...
          <tbody>
            {trainingPlanSummary.map((topic) => (
              <tr key={topic.topicName}>
                <td style={cellValue}>{topic.topicName}</td>
                <td style={cellValue}>{topic.count}</td>
                <td style={cellValue}>{topic.reason}</td>
                <td style={cellValue}>{topic.form}</td>
                <td style={cellValue}>{topic.verification}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <p>
          Aktuálne sa nezobrazujú žiadne sledované alebo rizikové oblasti pod
          hranicou 4,10. Podklad pre ročný plán vzdelávania nie je potrebné
          doplniť o nové rozvojové témy z anonymného hodnotenia.
        </p>
      )}

      <h2 style={sectionTitle}>6. Záver manažéra kvality</h2>

      {overallAverage !== null ? (
        <p>
          Celkové anonymné hodnotenie dosiahlo priemer{" "}
          <strong>{overallAverage.toFixed(2)}</strong>, čo zodpovedá úrovni{" "}
          <strong>{getScoreLevel(overallAverage).toLowerCase()}</strong>.
          Výsledky je vhodné použiť ako manažérsky podklad pre hodnotiace
          rozhovory, individuálne plány ďalšieho vzdelávania, ročný plán
          vzdelávania zamestnancov a identifikáciu oblastí, ktoré si vyžadujú
          metodické vedenie, zaškolenie alebo supervíziu.
        </p>
      ) : (
        <p>
          Zatiaľ nie sú dostupné žiadne odoslané hodnotenia. Záver manažéra
          kvality bude možné vytvoriť po prijatí prvých anonymných hodnotení.
        </p>
      )}

      <h2 style={sectionTitle}>7. Vyjadrenie / záver manažéra kvality</h2>
      ...
      >
        <div>
          ...
          <p style={{ marginTop: "6px", textAlign: "center" }}>Dátum</p>
        </div>
